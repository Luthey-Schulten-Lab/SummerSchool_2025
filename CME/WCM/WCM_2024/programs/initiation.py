...
def initializeConstants(sim_properties, genome3A):
    ...
    initializeriboTolocusMap(sim_properties)
...
def initializePromoterStrengths(sim_properties):
    ...
    for locusTag, locusDict in genome.items():    
        ...
        if locusDict["Type"] == 'protein':
            ...
            if 'S ribosomal protein' in PtnName:
                # Error here?? Corrected on 21thMay
                # For ribosomal proteins, the min is 500              
                PromoterStrengths[locusTag] = (max(25, PtnCount - 500) + 500)/180 
            else:
                PromoterStrengths[locusTag] = (min(765, max(45, PtnCount)))/180
                         
        elif locusDict["Type"] == 'tRNA':
            # PromoterStrengths[locusTag] = 180
            # Different from cell paper
            PromoterStrengths[locusTag] = 765/180

        elif locusDict["Type"] == 'rRNA':
            # Different from cell paper
            # Since 23S is 2900 nt long so give a higher promoter strength

            if locusTag in ['JCVISYN3A_0068', 'JCVISYN3A_0533']: # 23S
                PromoterStrengths[locusTag] = 765*6/180
            elif locusTag in ['JCVISYN3A_0069', 'JCVISYN3A_0534']: # 16S
                PromoterStrengths[locusTag] = 765*3/180
            elif locusTag in ['JCVISYN3A_0067', 'JCVISYN3A_0532']: # 5S
                PromoterStrengths[locusTag] = 765*2/180

        else:
            PromoterStrengths[locusTag] = 10/180
...
#########################################################################################
def initializeriboTolocusMap(sim_properties):
    """
    Input: 
